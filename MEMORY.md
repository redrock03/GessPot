...
### 2026-06-22 — שיפורי פוסט-השקה (שימוש חי של רן בנייד)

כל אלה נפרסו חי (gh-pages) + main, אומתו:
- **כפתור "חשב מחדש"** במסך המשחק — מרענן predictions/העשרה ומריץ את האנליסט שוב.
- **מסך "חיזוי" = לוח חיזויים** (`Predict.tsx`, route `/predict`; הוסר ה-placeholder הריק): "הניחושים שלך" + "הבאים בתור" (12 הקרובים, ✓ לשמור, ★ לערך-גבוה r16+). ללא הרצת Claude מראש.
- **הפרדה כיוון/בול בכרטיסים** (`SuggestionPair`): כיוון מוביל בנבחרת (מי מנצח, בלי תוצאה-כותרת, "תוצאה להגשה" משני); בול מוביל בתוצאה. תוויות "מי מנצח"/"התוצאה המדויקת". (`directionLabel` ב-format.ts.)
- **התאמת אגרסיביות לפי מצב הפול** (§4): `riskFromPoolGap` (פער חתום→α); store `poolAdjust/poolBehind/poolPoints`; הגדרות: טוגל "(אפשרות סימון)" + מקטע מאחור/מוביל + נקודות + אגרסיביות מומלצת. Match משתמש ב-α אפקטיבי; תג "מותאם למצב הפול" מסביר את ההטיה.
- **"מה צריך כל צד" גלוי במסך המשחק** — `describeNeed` לשתי הנבחרות + חיווי שהחיזוי כבר שוקלל לפי המוטיבציה (כבר הוזן לאנליסט; כעת שקוף).
- **ליינים של סוכנויות (odds)** — `/odds` bet=1; `marketProbs` (de-vig + ממוצע 14 בוקמייקרים → קונצנזוס) → `AnalystInput.odds` (Claude מכייל לשוק ומצטט: אומת 64/23/14 לארגנטינה-אוסטריה) + מוצג בפאנל "מודיעין". 119 בדיקות.
- **תזכורת חוב:** עדיין שווה — תקרת-הוצאה ב-Anthropic (הגנת-עלות), conduct→דירוג בתים, xG פר-משחק.
